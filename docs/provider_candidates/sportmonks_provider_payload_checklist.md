...
Minimal ID discovery, confirmed ID scope review and minimal field review have passed. The [transform design plan](sportmonks_transform_design_plan.md) is created docs-only. Cleanup remains pending; trial and implementation remain blocked. The next gate is a docs-only implementation-plan readiness decision.
